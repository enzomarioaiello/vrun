...
- `vrun new <module>` — generate RTL module and testbench scaffolding
  - `--no-tb` flag to generate only the module
  - `--tb-only` flag to generate only the testbench
- Generated modules include clock and active-low reset
- Generated testbenches come pre-wired with `$dumpfile`/`$dumpvars`
- Module name validation (must be a valid Verilog identifier)
